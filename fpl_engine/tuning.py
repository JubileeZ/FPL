...
async def _run_full_tuning_pipeline(current_config: dict, json_path: str, current_gw: int, n_trials_override: int = None):
    ...
    study_alphas.optimize(objective_alphas, n_trials=n_trials_override or 30, n_jobs=1, show_progress_bar=True, callbacks=[lambda study, trial: early_stopping_callback(study, trial, patience=15)])
    ...
    study_perf.optimize(objective_perf_idx, n_trials=n_trials_override or 50, n_jobs=1, show_progress_bar=True, callbacks=[lambda study, trial: early_stopping_callback(study, trial, patience=15)])
