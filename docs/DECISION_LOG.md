...
- **[ACCEPTED] Backup Anomaly Detection**: Generalized stand-in minutes inflation filter. **Why it was needed**: Backup players covering for injured starters were being misclassified as permanent starters due to compounded signals in the minutes engine (EMA inflation, loyalty triggers). **Solution**: Implemented a "Coverage Spike" Z-score filter (using a 3-fixture window exclusion) and team-positional depth hierarchy. This applies a tunable soft-reversion discount to minutes_IDX for confirmed stand-ins while preserving the integrity of genuine starters.
